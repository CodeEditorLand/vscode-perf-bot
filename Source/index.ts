...
async function runPerformanceTest(
	...
	if (opts.runtime === "web") {
		if (opts.quality === "stable") {
			if (opts.githubToken) {
				build =
					"https://vscode.dev/github/microsoft/vscode/blob/main/package.json";
			} else {
	...
	return new Promise((resolve) => {
		...
		log(
			`${chalk.gray("[exec]")} started npx process with pid ${chalk.green(npx.pid)}`,
		);
		...
		npx.on("error", (error) => {
			log(
				`${chalk.gray("[exec]")} failed to execute (${error.toString().trim()})`,
				true,
			);
		});
		...
		npx.on("close", (code, signal) => {
			log(
				`${chalk.gray("[exec]")} finished with exit code ${chalk.green(code)} and signal ${chalk.green(signal)}`,
			);
...
function parsePerfFile(): PerfData | undefined {
	...
	if (lines.length < 1) {
		log(
			`${chalk.red("[perf] found no performance results, refusing to send chat message")}`,
			true,
		);
...
async function sendSlackMessage(data: PerfData, opts: Opts): Promise<void> {
	...
	if (opts.slackMessageThreads) {
		...
		try {
			...
			log(
				`${chalk.gray("[perf]")} failed to load message threads from ${chalk.green(filepath)}`,
			);
		}
	}
...
module.exports = async function (argv: string[]): Promise<void> {
	...
	try {
		log(
			`${chalk.gray("[init]")} storing performance results in ${chalk.green(Constants.PERF_FILE)}`,
		);
